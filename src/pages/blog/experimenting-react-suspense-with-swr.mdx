...
import SuspenseWithSwrImage from 'public/blogs/experimenting-react-suspense-with-swr/Suspense_with_SWR.jpg';
import PostWithoutSuspenseGif from 'public/blogs/experimenting-react-suspense-with-swr/post-without-suspense.gif';
import PostWithSuspenseGif from 'public/blogs/experimenting-react-suspense-with-swr/post-with-suspense.gif';

export const meta = blogsMeta['experimenting-react-suspense-with-swr'];

<Img src={SuspenseWithSwrImage} width={720} height={450} priority />
...
SWR is a great data fetching library developed by [Vercel](https://vercel.com/). It uses a _"stale-while-revalidate"_ caching strategy and has Suspense support built-in.

Here is how _"stale-while-revalidate"_ works. If there is a cache for the data you fetch, the browser will first render the cached data. In the background, the browser fetches the new data and then swaps the old cached data when the new data comes in.
...
### Loading Indicator UI without Suspense
...
### Loading Indicator UI with Suspense
